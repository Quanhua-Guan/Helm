...
- (id)initWithDBName:(NSString *)theDBName
{
    if (self = [super init]) {
        dbName = theDBName;
        tableNames = [NSMutableArray array];
...
- (BOOL)deleteFromTable:(Class)tableClass whereField:(NSString *)field equalToValue:(id)value {
    NSString *tableName = [self checkBeforeQueryOrUpdateTable:tableClass];
    return [fmdb executeUpdate:[NSString stringWithFormat:@"DELETE FROM %@ WHERE %@=?", tableName, field], value];
}
...
- (BOOL)deleteFromTable:(Class)tableClass whereFields:(NSArray *)fields equalToValues:(NSArray *)values {
    ...
    //执行删除
    return [fmdb executeUpdate:sql withParameterDictionary:sqlParameterDic];
}
...
+ (BaseDBMasterKey *)defaultDBMasterKey {
    static BaseDBMasterKey *DBMasterKey = nil;
    if (DBMasterKey == nil) {
        DBMasterKey = [[BaseDBMasterKey alloc] init];
    }
    return DBMasterKey;
}
...
+ (BaseDBMasterKey *)DBMasterKeyWithDBName:(NSString *)dbName {
    ...
    static BaseDBMasterKey *DBMasterKey = nil;
    if ([dbName isEqual:DBName]) {
        if (DBMasterKey == nil) {
            DBName = dbName;
            DBMasterKey = [[BaseDBMasterKey alloc] initWithDBName:dbName];
        }
        return DBMasterKey;
    } else {
        DBName = dbName;
